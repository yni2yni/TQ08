// ADS Touch Sensor Test Example Program (IC P/N: TQ08 )
// Code:
// Date: 2021.02.08  Ver.: 0.0.3
...
#define Sensitivity5    0x0A //ch5
#define Sensitivity6  0x0B //ch6
#define Sensitivity7  0x0C //ch7
#define Sensitivity8  0x0D //ch8
...
#define Output1	0x10 //Touch Output Data Register 1Byte (OUT4~OUT1)
#define Output2	0x11 //Touch Output Data Register 1Byte (OUT8~OUT5)

#define Channel_Reset1	0x12 // Channel Reset Register(ch4~ch1),
//0: Enable Operation(Sensing + Calibration), 
//1: Reset Operation (No Sensing + Reset  Calibration)

#define Channel_Reset2	0x13 // Channel Reset Register(ch8~ch5),
//0: Enable Operation(Sensing + Calibration), 
//1: Reset Operation (No Sensing + Reset  Calibration)

#define Calibration_Hold1	0x14 // Calibration Holde Register(ch4~ch1),
//0: Enable Reference Calibration (Sensing + Calibration), 
//1: Disable Reference Calibration ( Sensing + No Calibration)

#define Calibration_Hold2	0x15 // Calibration Holde Register(ch8~ch5),
//0: Enable Reference Calibration (Sensing + Calibration), 
//1: Disable Reference Calibration ( Sensing + No Calibration)
...
#define CS5_Cap_Control	0x39 //CS5 Channel Internal Capacitaice Register
#define CS6_Cap_Control	0x3A //CS6 Channel Internal Capacitaice Register
#define CS7_Cap_Control	0x3B //CS7 Channel Internal Capacitaice Register
#define CS8_Cap_Control	0x3C //CS8 Channel Internal Capacitaice Register
...
void Register_Dump()
{
  ...
  for( int i =0; i < 256; i+=16 )
  {
    for (int j = 0; j <= 15; j++)
    {
      Wire.beginTransmission(TQ08_ID_GND); // sned ic slave address
      Wire.write((i+j));             // sends register address
      Wire.endTransmission();                // stop transmitting
      Wire.requestFrom(TQ08_ID_GND, 1);      // data read (1 byte)
...
void loop() {
  ...
  Wire.endTransmission(); // stop transmitting
  Wire.requestFrom(TQ08_ID_GND,2); // key data read (2 byte)
...
void  Init_TQ08(void)
{
  ...
  //   Wire.beginTransmission(TQ08_ID_GND);//
  ...
  Wire.beginTransmission(TQ08_ID_GND); //
  Wire.write(byte(Sensitivity5));      // 0xAh
  Wire.write(0x10);                    // HEX Value x 0.1% = 1.6%
  Wire.endTransmission();              //

  Wire.beginTransmission(TQ08_ID_GND); //
  Wire.write(byte(Sensitivity6));      // 0x0Bh
  Wire.write(0x10);                    // HEX Value x 0.1% = 1.6%
  Wire.endTransmission();              //

  Wire.beginTransmission(TQ08_ID_GND); //
  Wire.write(byte(Sensitivity7));      // 0x0Ch
  Wire.write(0x10);                    // HEX Value x 0.1% = 1.6%
  Wire.endTransmission();              //

  Wire.beginTransmission(TQ08_ID_GND); //
  Wire.write(byte(Sensitivity8));      // 0x0Dh
  Wire.write(0x10);                    // HEX Value x 0.1% = 1.6%
  Wire.endTransmission();              //
  ...
  Wire.write(0xF0);                    // 0: Sensing, 1: No Sensing (High 4bit = 1111)
  Wire.endTransmission();              //
